...
export function HomeCommunity() {
  return (
    <div className="relative min-h-[50vh] sm:min-h-[60vh] flex items-center justify-center bg-black border-t border-white/10">
            ...
            {socialLinks.map((item, index) => {
              ...
              return (
                <motion.a
                  ...
                  className="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-full border border-white/20 bg-white/[0.02] flex items-center justify-center hover:bg-white/10 hover:border-white/40 transition-all duration-300"
                  initial={{ opacity: 0, scale: 0.8 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  viewport={{ once: true }}
                >
                  <Icon className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 text-white/80" />
                </motion.a>
              )
            })}
          </motion.div>
