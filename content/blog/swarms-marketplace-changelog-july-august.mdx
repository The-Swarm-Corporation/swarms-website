...
- **Serious security and reliability work.** The free credit grant was hardened, purchases are now verified on chain, and a session bug that was silently signing people out mid-session was found and fixed.
...
- The image upload endpoint was hardened and now requires authentication.
...
- Security: the **free credit grant was hardened** and Cloudflare Turnstile was wired into the auth flow. The $5 grant is now gated by a captcha check, a per-IP daily cap, and a requirement that wallet-only accounts show prior on-chain history. Grants and denials are logged.
...
- **Free credit now requires a wallet with prior on-chain history**, tightening the eligibility rules introduced on August 4.
...
And underneath both, a lot of unglamorous work. The free credit grant hardened, purchases verified on chain, a session bug that was quietly logging people out found and killed, dead code cleared, and the whole site made usable on a phone.
