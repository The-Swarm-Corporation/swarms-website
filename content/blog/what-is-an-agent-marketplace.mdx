...
**Install and authenticate.** Install the framework and set the API key from [swarms.world/platform/api-keys](https://swarms.world/platform/api-keys):
...
The fastest way to understand an agent marketplace is to browse one. Explore agents, prompts, and tools at [swarms.world](https://swarms.world), publish your first component with a [free API key](https://swarms.world/platform/api-keys), and read the full marketplace documentation at [docs.swarms.ai](https://docs.swarms.ai). If you are building the components the agent economy runs on, the marketplace is where they find their market.
